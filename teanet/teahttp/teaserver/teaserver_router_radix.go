	...
	for {
		...
		if length < len(node.path) {
			newNode := &routerRadixNode{
				path:     strings.Clone(node.path[length:]),
				handler:  node.handler,
				indexes:  node.indexes,
				children: node.children,
			}
			node.path = strings.Clone(node.path[:length])
			node.handler = nil
			node.indexes = strings.Clone(newNode.path[0:1])
		...
		if length < len(path) {
			...
			newNode := &routerRadixNode{
				path:     strings.Clone(path[length:]),
				handler:  handler,
				indexes:  "",
				children: nil,
			}
		...
		if node.handler != nil {
			panic(fmt.Sprintf("teahttp: path `%s` conflicts", rawPath))
		}
