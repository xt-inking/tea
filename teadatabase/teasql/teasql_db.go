...
func NewDB(driverName string, dataSourceName string, config *teaconfig.SqlConfig, middleware ...teamiddleware.Middleware) *DB {
	...
	sqlBuildersMu.RLock()
	sqlBuilderi, ok := sqlBuilders[driverName]
	sqlBuildersMu.RUnlock()
	if !ok {
		panic(fmt.Errorf("teasql: unknown driver sqlBuilder %q (forgotten import?)", driverName))
	}
	var (
